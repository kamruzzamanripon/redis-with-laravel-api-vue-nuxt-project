<template>
  <div class="px-4 md:px-8 py-2 h-16 flex justify-between items-center shadow-sm bg-white">
    ...
    <div class="flex items-center">
      ...
      <div class="flex space-x-5 mr-6">
        <router-link to="/" exact-active-class="font-bold">Frontend</router-link>
        <router-link to="/backend" active-class="font-bold">Backend</router-link>
      </div>
      ...
      <div class="relative">
        ...
        <div v-show="open"
          ...
          ref="dropdown">
          <a href="#" class="block px-4 py-2 text-gray-600 hover:bg-gray-100 hover:text-blue-600">Edit Profile</a>
          <button href="#" class="block px-4 py-2 text-gray-600 hover:bg-gray-100 hover:text-blue-600" @click="logout">Sign Out</button>
        </div>
      </div>
    </div>
  </div>
...
export default {
  ...
  watch:{
    user(newVal, oldVal){
      this.title = this.user ? "$" +  this.user.revenue : "0"
    }
  }
};
</script>
